...
import tetris.data.H2Connection;
import tetris.data.TetrisRepository;
import tetris.messagehandlers.PlayerConsumer;
...
public class WebServer extends AbstractVerticle {
    ...
    private void initWebserver() {
        ...
        router.route("/tetris-00/socket/*").handler(new SocketHandler(vertx).create("tetris\\.events\\..+"));

        // JVD: voorbeeld van endpoint
        router.route("/tetris-00/api/status").handler(routingContext -> {
            JsonObject json = new JsonObject();
            json.put("status", "api online");
    ...
    private void initEventBus() {
        ...
        eb.consumer("tetris.events.player", playerConsumer::handler);
